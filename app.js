...
const CONFIG = {
  ...
  // Chave Pix
  pix: '71999999999',
  pixTipo: 'Telefone',   // Telefone | CPF | E-mail | Chave aleatória

  // Link de pagamento em cartão (débito ou crédito)
  // Cole aqui o link gerado pela sua maquininha / plataforma (Mercado Pago, PagBank, etc.)
  ...
  enderecoEstabelecimento: 'Rua das Pizzas, 123, Barra, Salvador, BA, Brasil',
  ...
  googleMapsKey: 'SUA_CHAVE_AQUI',

  // ─── TABELA DE PREÇO POR DISTÂNCIA ───────────────────────────
  // Cada faixa define: até quantos km → qual taxa em R$
  // Para alterar o valor de uma faixa, mude apenas o número após "taxa:"
  // Para adicionar uma faixa nova, copie uma linha e ajuste os valores
  tabelaDistancia: [
    { ate_km:  2, taxa:  5.00 },
    { ate_km:  4, taxa:  7.00 },
    { ate_km:  6, taxa:  9.00 },
    { ate_km:  8, taxa: 11.00 },
    { ate_km: 10, taxa: 13.00 },
    { ate_km: 13, taxa: 15.00 },
    { ate_km: 16, taxa: 18.00 },
    { ate_km: 20, taxa: 22.00 },
    // Acima de 20km — fora da área de entrega (deixe como está ou ajuste)
    { ate_km: 999, taxa: null },
  ],
...
function validarFormulario() {
  ...
  if (metodo === 'delivery') {
    const endOk = chk('fg-end', document.getElementById('f-end').value.trim());
  ...
  if (!ok && metodo !== 'delivery') showToast('⚠️ Preencha todos os campos obrigatórios');
  return ok;
}
